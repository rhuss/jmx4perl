 ...
 * @since Jul 10, 2009
 */
public class JmsSessionStatsHandler extends StatsHandler {

    @Override
    public Class getType() {
        return JMSSessionStats.class;
    }
    ...
    public Object extractObject(ObjectToJsonConverter pConverter,
        ...
        JMSSessionStats jStats = (JMSSessionStats) pValue;
        if (!pExtraArgs.isEmpty()) {
            String key = pExtraArgs.peek();
            if (key.equalsIgnoreCase("producers")) {
                pExtraArgs.pop();
                return pConverter.extractObject(jStats.getProducers(), pExtraArgs, jsonify);
            } else if (key.equalsIgnoreCase("consumers")) {
                pExtraArgs.pop();
                return pConverter.extractObject(jStats.getConsumers(), pExtraArgs, jsonify);
            } else {
            ...
            if (jsonify) {
                ...
                ret.put("producers",pConverter.extractObject(jStats.getProducers(),pExtraArgs,jsonify));
                ret.put("consumers",pConverter.extractObject(jStats.getConsumers(),pExtraArgs,jsonify));
                return ret;
            } else {
                return jStats;
            }
