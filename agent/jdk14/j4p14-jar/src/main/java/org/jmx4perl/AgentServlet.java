...
public class AgentServlet extends HttpServlet {
    ...
    private JSONObject getErrorJSON(int pErrorCode, Throwable pExp, JmxRequest pJmxReq) {
        ...
        jsonObject.put("status",new Integer(pErrorCode));
    ...
    private void registerRequestHandler() {
        ...
        for (int i = 0;i<handlers.length; i++) {
            requestHandlerMap.put(handlers[i].getType(),handlers[i]);
        }
    }
    ...
    private void registerOwnMBeans() {
        try {
            configMBeanName = mBeanServerHandler.registerMBean(configMBean,null);
