...
This document describes Version 0.20 of the j4p-agent protocol. This is the
initial version. 
...
specifying the search parameter like in 
   ...
   http://localhost:8080/j4p-agent/search/*:j2eeType=J2EEServer,*
...
minus, so j4p-agent can combine parts properly. The corresponding perl code for
creating such a format looks like
...
C<status> is always C<200>.  C<timestamp> contains the timestamp (i.e. epoch
seconds) when the attribute was read on the server. 
...
Example:
  ...
  http://localhost:8888/j4p-agent/search/*:j2eeType=J2EEServer,*
