...
WebLogic 9 & 10, Glassfish 2, Jonas 4 and Geronimo 2 as well as the pure
servlet containers Jetty 5 and 6, Tomcat 4, 5 & 6. Other platforms might work
...
IBM Websphere 6 & 7 (L<http://www.oracle.com/appserver/>)
...
The deployed agent servlet is quite mighty in the default installation as you
can perform any operation exposed by an MBean via JMX. Of course, you should
secure the agent via standard mechanism (e.g via Basich Authentication) to
allow only certain users access. But even then, you might not want you
monitoring guys to access the heart of the JEE Server. Therefor, you can
...
C<attribute> tag in which case only reading the attribute is allowed. 

=item 4.

Call C<ant>. Ah, yes, you need C<ant> installed (L<http://ant.apache.org>). To
be do an C<ant clean> before calling C<ant> without argument.
...
A typical (initial) workflow looks like
 ...
 $ jar tvf j4p.war
  ...
  85124 Thu Jul 30 10:42:22 CEST 2009 WEB-INF/lib/j4p-agent.jar
      0 Thu Jul 30 10:42:24 CEST 2009 WEB-INF/classes/j4p-agent.xml
