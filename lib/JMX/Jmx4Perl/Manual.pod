...
Additionally, there is a command line tool called (accidentally ;-) L<jmx4perl>
...
build. You can easily build it on your own by using C<mvn install> within the
F<agent/> subdirectory. You need maven (L<http://maven.apache.org>) as build
tool installed. The agent can be found afterwards at
F<agent/modules/j4p-mule/target/j4p-mule-0.70.0-agent.jar> (note the C<-agent>
...
For Mule 3.0 the C<custom-agent> declaration lives in the core mule namespace
(so don't use the C<management:> prefix within Mule 3.0). Don't forget to
include the proper namespace declarations as pointed out in the Mule
documentation. 
...
file. You have to rebuild the agent if you want to use this feature, though. To
do this, here's the recipe:
